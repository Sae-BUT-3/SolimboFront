    ...
    <>
      {items.length > 0 ? (
        <View style={styles.sectionFilter}>
          ...
          >
            <Text style={[screenStyle.filterText, filter === 'compilation']}>Compilations</Text>
          </Pressable>
          <Pressable
            style={[screenStyle.filterButton, filter === 'album' && { backgroundColor: Colors.DarkSpringGreen }]}
            onPress={() => setFilter('album')}
          >
            <Text style={[screenStyle.filterText, filter === 'album']}>Albums</Text>
          </Pressable>
        </View>
      ) : null}
      <FlatList
...
const EmptyList = () => (
  <View style={screenStyle.emptyListContainer}>
    <Text style={screenStyle.noItemsText}>Aucune apparution pour le moment.</Text>
  </View>
);
