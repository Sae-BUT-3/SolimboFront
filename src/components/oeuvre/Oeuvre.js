...
const Oeuvre = ({ data, artists, favoris, likeUser, setResponse, show }) => {
  ...
  const handleFavoris = () => {
    axiosInstance
      .post("/users/oeuvreFav", { idOeuvre: data.id, type: data.type })
      .then((res) => {
        if (!fav) {
          setFavoris(true);
        } else {
    ...
    data && (
      <ImageBackground
        ...
        style={[
          ...
          { marginBottom: data.type === "track" ? 30 : 0 },
        ]}
      >
              ...
              <View style={styles.section}>
                ...
                >
                  ...
                  {artists.length == 1 ? (
                    <Pressable
                      onPress={() =>
                        navigation.navigate("artist", { id: artists[0].id })
                      }
                    >
                      <Image
