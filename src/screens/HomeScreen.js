...
import { FlatList, Image, RefreshControl, ImageBackground, Platform, Pressable, StyleSheet, Text, View } from 'react-native';
    ...
    const updateReviews = () => {
        ...
        axiosInstance.get('/review/timeline', { params: { page: page, pageSize: 50, orderByLike: false } })
            .then(response => {
                ...
                setError(e.response?.data?.message || 'An error occurred while fetching reviews.');
            }).finally(() => {
                setIsLoading(false);
            });
    };
